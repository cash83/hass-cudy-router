...
async def test_async_setup_entry_adds_all_entities(
    ...
    await wr6500_sensor.async_setup_entry(hass, config_entry, _add_entities)

    assert len(added) == len(wr6500_sensor.SENSOR_DESCRIPTIONS)
...
def _desc_by_key(key: str):
    for d in wr6500_sensor.SENSOR_DESCRIPTIONS:
        if d.key == key:
            return d
...
def test_native_value_wan_key(coordinator: R700Coordinator, config_entry: MockConfigEntry):
    ...
    coordinator.data = {
        MODULE_WAN: {SENSOR_WAN_IP: "203.0.113.10"},
    }
